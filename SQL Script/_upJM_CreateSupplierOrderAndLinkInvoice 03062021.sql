...
BEGIN
	...
	DECLARE 		
		...
		@cStorno varchar(1),
	    @nOTPPantheonUsrID INT-- dodal BORIS
	/*LOG*/
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT TOP 1 
				...
				@cStorno = X.value('Storno[1]','VARCHAR(1)'),
				@nOTPPantheonUsrID = CAST(X.value('OTPPantheonUsrID[1]', 'INT') AS INT)  -- dodati cast za INT -- dodal BORIS
			...
			BEGIN
				...
				SET acCurrency = 'EUR', 
					...
					acDept = @cDept,
					acNote = @cNote,
					anClerk = @nOTPPantheonUsrID,
					anNoteClerk = @nOTPPantheonUsrID
				WHERE acKey = @p_cKey
			...
			-- èe obstaja vrednost v OrderNo 
			else
			begin
				
				IF (SELECT COUNT(*) FROM tHE_Order WHERE ackey = @cOrderNo) = 0
					SET @p_cError = 'Naroèilo ' + LTRIM(RTRIM(ISNULL(@cOrderNo, ''))) + ' ne obstaja!'

				IF ISNULL(@p_cError, '') <> ''
					RAISERROR(@p_cError, 16, 1)
				
				UPDATE tHE_Order
				SET acStatus = case when isnull(@cStorno,'') = '1' then 'S' else acStatus end,
				acConsignee = @cConsignee
				WHERE acKey = @cOrderNo
					
				/*Pridobivanje pozicij.*/
				DECLARE cPositions CURSOR LOCAL FAST_FORWARD FOR
					SELECT 
						X.value('Ident[1]', 'CHAR(16)'),
						X.value('Name[1]', 'VARCHAR(80)'),
					...
					FROM @p_XMLOrder.nodes('/TransportOrder/Products/Product') AS X(X) 
				OPEN cPositions
				FETCH cPositions INTO @cIdent, @cName, @nPrice
				WHILE @@fetch_status = 0
				BEGIN
					IF exists (select 1 from the_orderItem where ackey = @cOrderNo and acIdent = @cIdent)
					begin
						...
						select top 1 @nNo = anno from the_orderItem where ackey = @cOrderNo and acIdent = @cIdent order by anno
						
						/*Preraèun pozicije.*/
						WHILE isnull(@nNo,-1) > 0
						BEGIN
						   EXEC pHE_OrderItemInPVItem @cOrderNo, @nNo
						   
						   if exists (select 1 from the_orderItem where ackey = @cOrderNo and acIdent = @cIdent and anNo > @nNo )
							select top 1 @nNo = anno from the_orderItem where ackey = @cOrderNo and acIdent = @cIdent and anNo > @nNo order by anno
						   else
							set @nNo = -1
						END;
				...
				/*Preraèun glave.*/
				EXEC pHE_OrderHeadSetSum 'P', 'EUR', 'EUR', 0.01, 0.01, @cOrderNo	
