create procedure _upJM_OsveziVrednosti(	@p_cKey CHAR(13)) --Izhodni parameter s kljuèem dokumenta. )
as
begin
...
declare @nLID int,
    @cNarocKljuc char(13),
    @cLinkKljuc char(13),
    @nProcTrans decimal(25,9),
    @nFaktura decimal(25,9),
    @nNewProcTrans decimal(25,9),
    @nVrednostFakt decimal(25,9),
    @nZNNabave decimal(25,9),
    @nVrednostTrans decimal(25,9),
    @cNewKljuc char(13),
    @nVredTrans decimal(25,9),
    @nRVC decimal(25,9),
    @nVrednost decimal(25,9)
set @cNewKljuc = 'NEW'
set @nVrednostTrans = 0
set @nRVC = 0

declare cur cursor local fast_forward for
    select LID, NarocKljuc, LinkKljuc, ProcTrans, Vrednost
    from _epos_GTLink  
    where NarocKljuc =  @p_cKey
open cur
fetch next from cur into @nLID, @cNarocKljuc, @cLinkKljuc, @nProcTrans, @nVrednost
while @@fetch_status = 0 
begin
    if @cNewKljuc <> @cNarocKljuc
    begin
        select @nVrednostTrans = sum((anForPay - anVAT)/isnull(anFXrate,1)) from tHE_Order where acKey = @cNarocKljuc 
        select @nVrednostFakt = sum(anPVOCForPay - anPVOCVAT) from tHE_MoveItem where acKey in (select LinkKljuc from _epos_GTLink where NarocKljuc = @cNarocKljuc)
        set @cNewKljuc = @cNarocKljuc
    end
    select @nFaktura = sum(anPVOCForPay - anPVOCVAT) from tHE_MoveItem where acKey = @cLinkKljuc
    if @nFaktura = 0 
        set @nFaktura = 0.0001
    if isnull(@nVrednostFakt, 0) = 0
        set @nNewProcTrans = 0
    else
        set @nNewProcTrans = ((@nFaktura * 100)  / @nVrednostFakt)
    
    set @nVredTrans = (@nNewProcTrans * @nVrednostTrans) / 100
        
      
    update _epos_GTLink 
    set ProcTrans = @nNewProcTrans, 
        Vrednost = @nFaktura, 
        VredTrans = @nVredTrans,
        ProcTransFakt = ((@nVredTrans * 100)  / @nFaktura), 
        RVC = @nRVC, 
        ZNNabave = @nZNNabave
    where LID = @nLID
fetch next from cur into @nLID, @cNarocKljuc, @cLinkKljuc, @nProcTrans , @nVrednost
...
GO
/****** Object:  StoredProcedure [dbo].[DodajPantheonSupplierOrderAndLinkInvoice]    Script Date: 9. 02. 2021 13:44:43 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE OsveziPantheonLinkedInvoicesByOrderNo
(
      @p_orderNo CHAR(13) 
)
AS
BEGIN
      EXEC Grafolit55SI.dbo._upJM_OsveziVrednosti @p_orderNo
END
...
CREATE PROCEDURE [dbo].[_upJM_CreateSupplierOrderAndLinkInvoice]
(
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET @cOrderNoFacKey = @cOrderNoFac
...
/****** Object:  StoredProcedure [dbo].[SeznamPovezanihFakturByOrderNo]    Script Date: 17. 02. 2021 13:12:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SeznamPovezanihFakturByOrderNo](@p_orderNo CHAR(13))
AS
...
ALTER TABLE OdpoklicKupecPozicija add Akcija int;
ALTER TABLE OdpoklicKupecPozicija add acKey varchar(50);
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[DodajPantheonSupplierOrderAndLinkInvoice]
(
      @p_XMLOrder XML,
      @p_XMLInvoice XML,
      @p_cKey CHAR(13) OUTPUT,
      @p_cError VARCHAR(8000) OUTPUT
)
AS
BEGIN
      EXEC Grafolit55SI.dbo._upJM_CreateSupplierOrderAndLinkInvoice @p_XMLOrder, @p_XMLInvoice, @p_cKey OUTPUT, @p_cError OUTPUT
END
